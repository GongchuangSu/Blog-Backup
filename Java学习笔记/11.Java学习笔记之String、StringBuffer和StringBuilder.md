...
if(obj1 == obj2)
	...
	System.out.println(false);
```
输出为`true`。这是因为程序在运行的时候会创建一个字符串缓冲池(也就是栈内存)。当使用 `String obj1  = "xyz";` 这样的表达是创建字符串的时候（非`new`这种方式），程序首先会在这个 `String` 缓冲池中寻找相同值的对象，在 `String str1 = "xyz";` 中，`obj1`  先被放到了池中，所以在 `obj2` 被创建的时候，程序找到了具有相同值的 `str1`并将 `s2` 引用 `s1` 所引用的对象 `"xyz"`。
